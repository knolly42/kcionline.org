...
{
    [DisplayName( "KCI Group Attendance Detail" )]
    ...
    public partial class KCIGroupAttendanceDetail : RockBlock
    {
        ...
        {
            BindSchedules( ExtensionMethods.SelectedValueAsInt(ddlLocation) );
        }
        ...
        private ScheduleOccurrence GetOccurrence()
        {
            ...
            if ( Page.IsPostBack && _allowAdd )
            {
                ...
                if ( !locationIds.Any( l => l != 0 ) && ExtensionMethods.SelectedValueAsInt(ddlLocation).HasValue )
                {
                    locationId = ExtensionMethods.SelectedValueAsInt(ddlLocation).Value;
                    locationIds = new List<int> { locationId.Value };
                }

                if ( !scheduleIds.Any( s => s != 0 ) && ExtensionMethods.SelectedValueAsInt(ddlSchedule).HasValue )
                {
                    scheduleId = ExtensionMethods.SelectedValueAsInt(ddlSchedule).Value;
        ...
        {
            string script = string.Format( (string) @"
...
", (object) cbDidNotMeet.ClientID );

            ScriptManager.RegisterStartupScript( (Control) cbDidNotMeet, cbDidNotMeet.GetType(), "group-attendance-detail", script, true );
        }
