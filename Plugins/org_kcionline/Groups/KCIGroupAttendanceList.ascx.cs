...
{
    [DisplayName( "KCI Group Attendance List" )]
    ...
    public partial class KCIGroupAttendanceList : RockBlock
    {
        ...
        protected void gOccurrences_RowDataBound( object sender, GridViewRowEventArgs e )
        {
            if ( e.Row.RowType == System.Web.UI.WebControls.DataControlRowType.DataRow )
            {
                ...
                {
                    var deleteField = Enumerable.OfType<DeleteField>(gOccurrences.Columns).First();
        ...
        protected void gOccurrences_Add( object sender, EventArgs e )
        {
            ...
            {
                int? locId = ExtensionMethods.SelectedValueAsInt(ddlLocation);
        ...
        protected void BindGrid()
        {
            if ( _group != null )
            {
                ...
                if ( ddlSchedule.Visible && ddlSchedule.SelectedValue != "0" )
                {
                    scheduleIds.Add( ExtensionMethods.SelectedValueAsInt(ddlSchedule) ?? 0 );
                }

                var qry = new ScheduleService( _rockContext )
                    .GetGroupOccurrences( _group, fromDateTime, toDateTime, locationIds, scheduleIds, true, ExtensionMethods.SelectedValueAsInt(bddlCampus) )
                    .AsQueryable();
