...
{
    /// <summary>
    /// A Search Controller that provides people with access to people below them in their line and also their line member's followups
    /// </summary>
    public class LineAndFollowUpsController : Rock.Rest.ApiController<Person>
    {
        public LineAndFollowUpsController() : base( new PersonService( new RockContext() ) ) { }
...
";
        ...
        private IQueryable<Person> GetByFullName( string fullName, bool includeDeceased, bool includeBusinesses, bool allowFirstNameOnly, out bool reversed )
        {
            ...
            var allowedPersons = Utils.LineQuery.GetLineMembersAndFollowUps( personService, GetPerson(), rockContext, true );
